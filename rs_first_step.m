...
for i=1:length(filenames)
    ...
    elseif ~isempty(strfind(file, 'R2'))
        ...
        min_time = min(cellfun(@min, data.time)); % obtengo tiempo minimo de la se�al original
        max_time = max(cellfun(@max, data.time)); % obtengo tiempo maximo de la se�al original
        % ese es el rango de tiempo en el cual se mueve la se�al

        % divido la se�al en dos partes iguales, una para open eyes y otra para
        % close eyes
        tmax = max_time/2;
        cfg = [];
